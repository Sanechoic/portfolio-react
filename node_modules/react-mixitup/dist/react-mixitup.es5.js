import { memo, forwardRef, createElement, useMemo, useReducer, useRef, useEffect, useCallback, Fragment } from 'react';
 ...
 * @class ReactMixitup
 */
var OuterBound = memo(forwardRef(function (props, ref) { return (createElement("div", __assign({ style: { position: 'relative' } }, props, { ref: ref }))); }));
function init() {
    return {
        animate: false,
        commit: false,
...
var makeAbsoluteWrapper = function (Wrapper) {
    return memo(forwardRef(function (props, ref) { return (createElement(Wrapper, __assign({ style: {
            position: 'absolute',
            top: '0',
            left: '0',
            right: '0',
...
var makeWrapper = function (ReactMixitupWrapper) {
    return memo(forwardRef(function (props, ref) { return createElement(ReactMixitupWrapper, __assign({}, props, { ref: ref })); }));
};
var ReactMixitup = memo(forwardRef(function (_a, outerBoundRef) {
    ...
    var Wrapper = useMemo(function () { return makeWrapper(ReactMixitupWrapper); }, [ReactMixitupWrapper]);
    var AbsoluteWrapper = useMemo(function () { return makeAbsoluteWrapper(Wrapper); }, [Wrapper]);
    var items = unparsedItems.map(function (key) { return key.toString(); });
    var _e = useReducer(reducer, items, init), _f = _e[0], hash = _f.hash, animate = _f.animate, mount = _f.mount, commit = _f.commit, unsafeDispatch = _e[1];
    ...
    var measureRef = useCallback(function (key, el) {
    ...
    var wrapperMeasureContainerHeight = useCallback(function (el) {
        if (!el) {
            return;
        }
        refs.current.states[refs.current.states.length - 1].containerHeight = el.offsetHeight;
    }, []);
    var rows = useCallback(function (_a) {
    ...
    var measureNewGrid = newGrid ? (createElement(AbsoluteWrapper, { ref: wrapperMeasureContainerHeight }, rows({
        itemsToRender: currentState.items,
        ref: measureRef
    }))) : null;
    if (!transition) {
        return (createElement(OuterBound, { ref: outerBoundRef },
            createElement(Wrapper, null, rows({
                itemsToRender: currentState.items
            }))));
    }
    var child = (createElement(Fragment, null,
        createElement(Wrapper, null, rows({
            itemsToRender: currentState.items
        })),
        measureNewGrid));
    ...
    var allItems = lodash_flatten(states.map(function (_a) {
        var sItems = _a.items;
        return sItems;
    }));
    ...
    var animationRenderItems = lodash_uniq(allItems)
        .sort()
        .filter(getLatestPositions);
    if (mount) {
        // debugger;
        child = (createElement(Fragment, null,
            createElement(Wrapper, { style: { height: states[states.length - 2].containerHeight + 'px' } }, rows({
    ...
    if (animate) {
        // debugger;
        child = (createElement(Fragment, null,
            createElement(Wrapper, { style: { height: currentState.containerHeight + 'px' } }, rows({
    ...
    refs.current.persistedElement = createElement(OuterBound, { ref: outerBoundRef }, child);
    return refs.current.persistedElement;
}));
